class Login {
  ...
  static endpoint = "https://loginv1.uosp3.repl.co/"
  //https://loginv1.uosp3.repl.co/?matricula=123&senha=321
  ...
  static login = (callback_ok, callback_naook, config = null) => {
    ...
    this.estilocss =
      ...
      ".elementosLogin{display:flex; justify-content:center; align-items:flex-start; flex-direction:column; width:50%; background-color:#eee; padding:10px; border-radius:10px 0px 0px 10px; box-sizing:nherit;}" +
  ...
  static verificaLogin = () => {
    ...
    const endpoint = `https://loginv1.uosp3.repl.co/?matricula=${mat}&senha=${pas}`
    fetch(endpoint)
      .then((res) => res.json())
      .then((res) => {
        if (res) {
