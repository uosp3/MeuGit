    ...
    .then((res) => {
      ...
      res.forEach((el) => {
        ...
        imgDelete.addEventListener("click", (evt) => {
          const id = evt.target.parentNode.parentNode.firstChild.innerHTML; //pegar o id a deletar
          const linha = evt.target.parentNode.parentNode; //pegar a linha a deletar do dom
        ...
        imgEditar.addEventListener("click", (evt) => {
          document.querySelector("#janelaEditar").classList.remove("ocultar"); //exibir a janelaEditar
          const id = evt.target.parentNode.parentNode.firstChild.innerHTML; //pegar o id a editar
        ...
        imgExibir.addEventListener("click", (evt) => {
          document.querySelector(".janelaView").classList.remove("ocultar"); //exibir a janelaView
          const id = evt.target.parentNode.parentNode.firstChild.innerHTML; //pegar o id a exibir
          const endpoint = `http://127.0.0.1:1880/produto/${id}`;
          fetch(endpoint)
            .then((res) => res.json())
            .then((res) => {
...
document.querySelector("#btn_gravar").addEventListener("click", (evt) => {
  const id = document.querySelector("#f_ideditar").value; //pegar o id a exibir
