...
CAMINHO_HTML = pathlib.Path(__file__).parent / '303 email.html'
...
with open(CAMINHO_HTML, 'r') as arquivo:
    ...
    texto_email = template.substitute(nome='Edson')
...
try:
    with smtplib.SMTP(smtp_server, smtp_port) as server:
        server.ehlo()  # extender helo
        server.starttls()  # iniciar conexão segura
        server.login(smtp_username, smtp_password)
        server.send_message(mime_multipart)
        print('E-mail enviado com  sucesso!')
except Exception:
    print("Algo deu errado, verifique usuário e senha e tente novamente")
